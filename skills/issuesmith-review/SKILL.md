...
├──────────────────────────────────────────────────────────────┤
│                                                               │
│  最终验证 → AC 检查 → 清理检查 → 推送 → 创建 PR → CI 等待    │
│                                                       │      │
│                    ┌──────────────────────────────────┘      │
│                    ▼                                          │
│              处理 Review 意见                                │
...
PR 创建后，**GitHub CI 通常会触发自动检查**（测试、lint、构建等）。必须等待 CI 完成。

```bash
gh pr checks <PR 编号>
```

**MANDATORY:** 在 CI 全部通过之前，不执行下一步。
...
CI 通过后，让 AI 做一次自检 review：

```
请 review 当前 PR #N 的变更，关注：
...
Critical 和 Important 问题必须修复后才能合并。修复后重新推送，**重新等待 CI 通过**。
